export function scenarioScore(props, scenario) {
  ...
  const wPm25  = Number(props.w_pm25   ?? 1.0)
  const wPov   = Number(props.w_poverty ?? 0.8)
  const wAcc   = Number(props.w_access  ?? -0.7)
  ...
  return Math.min(1, Math.max(0, base + wPm25 * dPm25 + wPov * dPoverty + wAcc * dAccess))
}
